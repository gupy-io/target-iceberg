...
class TargetRemoveColumnsTest(TargetFileTestTemplate):
    ...
    def test(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match=r"Target schema's field names are not matching the table's field names: \['id', 'column_to_be_filter', 'created_at'\], \['id', 'created_at'\]",
        ):
...
class TargetPartitioningTest(TargetFileTestTemplate):
    ...
    def test(self) -> None:
        ...
        ), f"Expected partitions 3, but got {number_of_partitions}"
